...
parser.add_argument('--content_dir', type=str, required=True,
                    help='Directory path to a batch of content images')  # 内容图像路径
parser.add_argument('--style_dir', type=str, required=True,
                    help='Directory path to a batch of style images')  # 风格图像路径
parser.add_argument('--vgg', type=str, default='models/vgg_normalised.pth')  # VGG预训练模型

# training options
parser.add_argument('--save_dir', default='./experiments',
                    help='Directory to save the model')  # 模型保存路径
parser.add_argument('--log_dir', default='./logs',
                    help='Directory to save the log')  # 日志保存路径
parser.add_argument('--lr', type=float, default=1e-4)  # 学习率
parser.add_argument('--lr_decay', type=float, default=5e-5)  # 学习率衰变
...
content_dataset = FlatFolderDataset(args.content_dir, content_tf)  # 加载内容图片
style_dataset = FlatFolderDataset(args.style_dir, style_tf)  # 加载风格图片
